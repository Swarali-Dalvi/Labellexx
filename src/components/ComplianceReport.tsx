...
export const ComplianceReport: React.FC<ComplianceReportProps> = ({
  ...
  // Send to Manufacturer State & Contact Extraction
  ...
  const consumerCareField = report.fieldResults.find(f => f.ruleId === 'consumer_care');
  const mfgAddressField = report.fieldResults.find(f => f.ruleId === 'mfg_address');

  const rawContactText = `${consumerCareField?.extractedRawText || ''} ${consumerCareField?.normalizedValue || ''} ${mfgAddressField?.extractedRawText || ''} ${report.mergedRawOcrText}`;
  const extractedEmailMatch = rawContactText.match(/[\w.-]+@[\w.-]+\.[a-zA-Z]{2,}/);
  const manufacturerEmail = extractedEmailMatch ? extractedEmailMatch[0] : 'customercare@manufacturer.com';

  const extractedPhoneMatch = rawContactText.match(/(?:1800[-\s]?\d{3}[-\s]?\d{4}|\+?91[-\s]?\d{10}|\b\d{3,5}[-\s]?\d{6,8}\b)/);
  const manufacturerPhone = extractedPhoneMatch ? extractedPhoneMatch[0].replace(/\s+/g, '') : '1800-425-4449';

  const failedOrMissingFields = report.fieldResults.filter(
    f => f.status === 'violation' || f.status === 'non_standard' || f.status === 'review'
  );

  const failedFieldsSummary = failedOrMissingFields.length > 0
    ? failedOrMissingFields.map(f => {
        const issueType = f.status === 'violation' ? 'Missing / Mandatory Clause Violated' : f.status === 'non_standard' ? 'Non-Standard Phrasing' : 'Requires Review / Verification';
        return `• ${f.fieldName}: ${issueType} (${f.notes || f.legalReference})`;
      }).join('\n')
    : '• All mandatory statutory declarations are verified and compliant.';

  const handleSendToManufacturerEmail = () => {
    const subject = `[Legal Metrology Compliance Report] ${report.productName} (Ref: ${report.id})`;
    const body = `Dear Manufacturer / Compliance Team,\n\nHere is the statutory Legal Metrology (Packaged Commodities) Rules, 2011 compliance assessment report for your packaged commodity:\n\nProduct: ${report.productName}\nBrand: ${report.brandName || 'N/A'}\nAudit Verdict: ${report.overallStatus} (Score: ${report.overallScore}%)\nInspection Timestamp: ${new Date(report.timestamp).toLocaleString()}\n\nDeficiencies / Non-Compliant / Missing Fields:\n${failedFieldsSummary}\n\nPlease take corrective action to standardize the packaging declarations in accordance with Legal Metrology mandates.\n\nGenerated by LabelLex Statutory Compliance Engine.`;
    
    window.open(`mailto:${manufacturerEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`, '_blank');
    setIsSendMenuOpen(false);
    setSentConfirmation(`Report sent to manufacturer (${manufacturerEmail})`);
    setTimeout(() => setSentConfirmation(null), 5000);
  };

  const handleSendToManufacturerMessage = () => {
    const messageText = `*Legal Metrology Compliance Audit Report*\nProduct: *${report.productName}*\nBrand: *${report.brandName || 'N/A'}*\nVerdict: *${report.overallStatus}* (Score: ${report.overallScore}%)\nRef ID: ${report.id}\n\n*Deficiencies / Missing Fields:*\n${failedFieldsSummary}\n\n_Generated by LabelLex Statutory Compliance Engine_`;
    
    const cleanNum = manufacturerPhone.replace(/[^\d+]/g, '');
    const waUrl = `https://wa.me/${cleanNum.startsWith('91') || cleanNum.startsWith('+') ? cleanNum.replace('+', '') : '91' + cleanNum}?text=${encodeURIComponent(messageText)}`;
    
    window.open(waUrl, '_blank');
    setIsSendMenuOpen(false);
    setSentConfirmation(`Report sent to manufacturer (${manufacturerPhone})`);
    setTimeout(() => setSentConfirmation(null), 5000);
  };
  ...
  return (
    <div className="bg-white rounded-3xl border border-charcoal-200 shadow-soft-lg p-6 sm:p-10 fade-in space-y-8">
      ...
      {/* Role-Specific Action Buttons & Send to Manufacturer */}
      {showRoleActions && (
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-charcoal-200 no-print">
          ...
          {/* Bottom-Right Corner: Send to Manufacturer */}
          <div className="relative ml-auto">
            <div className="flex items-center space-x-2">
              {sentConfirmation && (
                <div className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold animate-fadeIn">
              ...
              <button
                ...
                className="flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-charcoal-900 text-xs font-bold shadow-soft-md hover:scale-102 active:scale-98 transition-all cursor-pointer"
              >
                <Send className="w-4 h-4" />
                <span>Send to Manufacturer</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isSendMenuOpen ? 'rotate-180' : ''}`} />
              </button>
            </div>

            {/* Dropdown with Email and Message Options */}
            {isSendMenuOpen && (
              <div className="absolute right-0 bottom-full mb-2 w-72 rounded-2xl bg-white border border-charcoal-200 shadow-soft-2xl p-2 z-40 animate-fadeIn space-y-1">
                <div className="px-3 py-2 border-b border-charcoal-100">
                  <p className="text-xs font-extrabold text-charcoal-900">Send to Manufacturer</p>
                  <p className="text-[10px] text-charcoal-500">
                    Includes {failedOrMissingFields.length} flagged fields & report breakdown
                  </p>
                </div>

                <button
                  type="button"
                  onClick={handleSendToManufacturerEmail}
                  className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl hover:bg-cream-100 text-left transition-colors cursor-pointer"
                >
                  <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">
                    <Mail className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-bold text-charcoal-900">Email</div>
                    <div className="text-[10px] text-charcoal-500 truncate">{manufacturerEmail}</div>
                  </div>
                </button>

                <button
                  type="button"
                  onClick={handleSendToManufacturerMessage}
                  className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl hover:bg-cream-100 text-left transition-colors cursor-pointer"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-bold text-charcoal-900">Message (WhatsApp / SMS)</div>
                    <div className="text-[10px] text-charcoal-500 truncate">{manufacturerPhone}</div>
                  </div>
                </button>
              </div>
            )}
